        ...
        private void Awake()
        {
            ...
            if (triggerCollider == null)
            {
                throw new MissingComponentException(
                    $"{nameof(AttackSensor)} on '{gameObject.name}' requires a {nameof(Collider2D)} component.");
            }

            if (!triggerCollider.isTrigger)
            {
                throw new InvalidOperationException(
                    $"{nameof(AttackSensor)} on '{gameObject.name}' requires its {nameof(Collider2D)} to be a trigger.");
            }
        }

        public bool TryGetNearestTarget(out Collider2D targetCollider, out IDamageReceiver damageReceiver)
        {
            targetCollider = null;
            damageReceiver = null;
            ...
            for (var i = _overlaps.Count - 1; i >= 0; i--)
            {
                ...
                var receiver = overlap.GetComponent<IDamageReceiver>();
                if (receiver == null)
                {
                    continue;
                }
